...
def create_orchestrator():
    ...
    try:
        ...
        if api_key and api_key != "not-configured":
            return LangGraphEngine()
    except ImportError:
        pass
...
async def _tool_direct_agent_node(self, state: AgentState):
    # 直接调用工具（无LLM参与）
...
```python
expected_token = os.getenv("DEFAULT_TOKEN")
